...
class ToneMapper:
    def __init__(self, device: spy.Device, max_levels: int = 16):
        self.device = device
        if max_levels < 1:
    ...
    def prepare_weights(self, encoder, source, exposure_ev, highlight_ev, shadow_ev, sigma=0.2):
        ...
        key = (source, exposure_ev, highlight_ev, shadow_ev, sigma)
        if key == self._weight_key:
            return
        if self.base_color is None or (self.base_color.width, self.base_color.height) != (source.width, source.height):
            levels = fusion_mip_count(source.width, source.height, self.max_levels)
            self.base_color = self.create_texture(source.width, source.height)
            self.luminance_pyramid = self.create_texture(source.width, source.height, levels=levels)
    ...
    def prepare_result(self, encoder, hdr_source, exposure_ev):
        if self._result_key == self._weight_key:
            return
        if self.reconstructed is None or (self.reconstructed.width, self.reconstructed.height) != (hdr_source.width, hdr_source.height):
            self.reconstructed = self.create_texture(
                hdr_source.width, hdr_source.height, spy.Format.r32_float,
                levels=self.luminance_pyramid.mip_count)
            self.local_exposure = self.create_texture(hdr_source.width, hdr_source.height, spy.Format.r32_float)
            self.final_color = self.create_texture(hdr_source.width, hdr_source.height)
        ...
        for mip in range(last, -1, -1):
            ...
            previous = self.local_exposure if mip == last else self.reconstructed.create_view(mip=mip+1, mip_count=1)
            self.reconstruction_kernel.dispatch(
                thread_count=[max(1, hdr_source.width >> mip), max(1, hdr_source.height >> mip), 1],
